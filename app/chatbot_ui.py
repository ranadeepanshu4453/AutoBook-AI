...
if "messages" not in st.session_state:
    ...
    st.session_state.started        = False   # True after user says yes
...
def render_car_cards_native(cars, msg_idx=0):
    if not cars: return
...
def render_bot_extras(cars, entities, msg_idx=0):
...
def send_to_api(query: str):
    """Call API and return (msg_text, cars, entities, stage_lbl)."""
    try:
        ...
        return "Cannot connect — make sure the API is running on port 8000.", [], {}, ""
...
def append_and_render_bot(msg_text, cars, entities, stage_lbl):
    """Render bot message and store in history."""
    if stage_lbl:
    ...
    st.session_state.messages.append({
        ...
        "cars":cars,"entities":entities,"stage":stage_lbl,
    })
...
# ── Welcome message (always shown, static) ────────────────────────────
with st.chat_message("assistant"):
    st.markdown("""
    <div class="welcome-msg">
        👋 Hi there! I'm your Car Rental Assistant.<br><br>
        I can help you find and book the perfect car for your trip — just tell me your dates and preferences and I'll do the rest.<br><br>
...
if st.session_state.pending_query:
    ...
    with st.chat_message("assistant"):
        with st.spinner(""):
            msg_text, cars, entities, stage_lbl = send_to_api(query)
...
# ── Date picker widget (shown when stage = collecting_dates) ──────────
if st.session_state.started and st.session_state.current_stage == "collecting_dates":
    st.markdown('<div class="date-picker-box"><div class="dp-title">Select Your Dates</div></div>', unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        pickup = st.date_input("Pickup Date", min_value=TODAY, value=TODAY, key="pickup")
    with col2:
        dropoff = st.date_input("Drop-off Date", min_value=pickup + datetime.timedelta(days=1),
                                value=pickup + datetime.timedelta(days=1), key="dropoff")
    if st.button("Search Available Cars →", use_container_width=True):
        ...
        with st.chat_message("assistant"):
            with st.spinner(""):
                msg_text, cars, entities, stage_lbl = send_to_api(date_query)
            append_and_render_bot(msg_text, cars, entities, stage_lbl)
        st.rerun()

# ── Chat input ────────────────────────────────────────────────────────
if prompt := st.chat_input("Type a message..."):
    # Check if user is saying yes to start
    if not st.session_state.started and prompt.strip().lower() in YES_WORDS:
        ...
        # Not started yet and didn't say yes
        with st.chat_message("assistant"):
        ...
        with st.chat_message("assistant"):
            with st.spinner(""):
                msg_text, cars, entities, stage_lbl = send_to_api(prompt)
            append_and_render_bot(msg_text, cars, entities, stage_lbl)
        st.rerun()